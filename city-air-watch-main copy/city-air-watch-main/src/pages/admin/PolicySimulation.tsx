    ...
    <div className="space-y-6 container mx-auto pb-10">
      ...
      >
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Policy Simulation Lab</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="bg-muted/30 px-3 py-1.5 rounded-xl border border-border/50">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Scenario Mode</span>
          </div>
            ...
            <div className="flex items-center justify-between">
              {typeof stat.val === 'number' ? (
                <motion.h4
                  ...
                  className={`text-4xl font-bold font-mono ${stat.col} `}
                >
                  {stat.isAnimated ? "..." : stat.val}
                </motion.h4>
              ) : (
                <h4 className={`text-4xl font-bold font-mono ${stat.col} `}>{stat.val}</h4>
              )}
              {stat.label.includes("Target") && simulationComplete && simulatedAQI! < currentAQI && (
                <div className="bg-emerald-500/10 text-emerald-400 p-2 py-1 rounded-lg text-xs font-bold border border-emerald-500/20">
          ...
          <div className="p-8 rounded-[2.5rem] bg-card border border-border shadow-2xl">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={resetSimulation} size="sm" className="rounded-xl border-border/40 text-[10px] font-bold uppercase tracking-wider h-9 bg-muted/20">
                  ...
                  >
                    <div className="flex items-center justify-between mb-4 relative z-10">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-foreground uppercase tracking-tight leading-none mb-1">{action.name}</p>
                          <p className="text-[10px] font-medium text-muted-foreground">IMPACT: {action.impact} AQI</p>
                        </div>
                      </div>
                      <Switch
                    ...
                    <AnimatePresence>
                      {action.enabled && (
                        ...
                        >
                          <div className="flex justify-between items-center text-[10px] uppercase font-bold tracking-wider text-primary">
                          ...
                          <div className="flex justify-between items-center bg-card/60 rounded-xl p-2 px-3 border border-primary/10">
                            <span className="text-[10px] font-medium text-muted-foreground">Scenario Cost</span>
                            <span className="text-xs font-bold text-foreground">₹{action.cost.toLocaleString()}</span>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
              ...
              >
                {isSimulating ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-lg uppercase tracking-tight">Running Simulation Matrix...</span>
                  </div>
                ) : (
                  <>
                    <Play className="w-6 h-6 fill-current transition-transform group-hover:scale-110" />
                    <span className="font-bold text-lg uppercase tracking-tight">Execute Digital Twin Simulation</span>
        ...
        >
          <div className="p-7 rounded-[2.5rem] bg-card/80 border border-border shadow-2xl backdrop-blur-xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-bold uppercase tracking-wider">Projection Summary</h3>
            </div>
            ...
            {enabledActions.length > 0 ? (
              <div className="space-y-4">
                {[
                  ...
                  { l: "Projected Result", v: estimatedAQI, c: "text-primary font-mono text-xl font-bold" }
                ].map(r => (
              ...
              <div className="flex flex-col items-center justify-center py-10 opacity-30">
                ...
                <p className="text-xs font-bold uppercase tracking-wider text-center">Awaiting Configuration</p>
              </div>
            )}
          </div>
          ...
          <div className="p-6 rounded-[2rem] bg-gradient-to-br from-primary/5 to-violet-500/5 border border-primary/20">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h4 className="text-[10px] font-bold uppercase tracking-wider text-primary">Simulation Methodology</h4>
            </div>
        ...
        {simulationComplete && (
          ...
          >
            <div className="flex items-center justify-between mb-10 px-2">
              <div>
                <h3 className="text-2xl font-bold tracking-tight uppercase">Sector Impact Assessment</h3>
            ...
            <div className="mt-8 flex items-center justify-center gap-10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Pre-Intervention</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-primary" />
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Effective Outcome</span>
              </div>
            </div>
          </motion.div>
        )}
